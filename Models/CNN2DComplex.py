...
class CNN2DComplex:
    ...
    class Trainer:
        def __init__(self, input_shape, output_dim):
            ...
            model.add(layers.InputLayer(input_shape=(input_shape[1],input_shape[2],1)))
            # Couche de convolution 1D avec 32 filtres, une fenêtre de 3 et une fonction d'activation ReLU
            model.add(layers.Conv2D(12, kernel_size=(2, 3), activation=layers.LeakyReLU(alpha=0.1),padding='valid'))
            model.add(layers.Conv2D(12, kernel_size=(2, 3), activation=layers.LeakyReLU(alpha=0.1), padding='valid'))
            model.add(layers.Conv2D(6, kernel_size=(2, 3), activation=layers.LeakyReLU(alpha=0.1), padding='valid'))
...
if __name__ == "__main__":
    data_loader = DataLoader("C:/Users/Younes srh/Desktop/I3/ProjetRadarIA/Data/Dataset_X2312_SNR.csv","C:/Users/Younes srh/Desktop/I3/ProjetRadarIA/Data/Dataset_y2312_SNR.csv")
    data, labels = data_loader.load_data()
    radar_dataset = RadarDataSet(data, labels, 0.4, appended_snr= True)

    trainer = CNN2DComplex.Trainer(data_to_complex(radar_dataset.X_train).shape, 181)
    ...
    trainer.saveModel("CNN2DComplex_1")
