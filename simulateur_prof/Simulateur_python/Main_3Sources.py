...
if __name__ == "__main__":
    ...
    for SNR in range(-5, 30, 1):
        ...
        for i in range(100):
            M = random.randint(2,3)  # Nombre de cible
            R = np.array([[random.randint(10, 30)] for i in range(M)])
    ...
    Dataset_X.to_csv(f'Dataset_X{randomfileid}_3-2S.csv', index=False)
    Dataset_y.to_csv(f'Dataset_y{randomfileid}_3-2S.csv', index=False)
